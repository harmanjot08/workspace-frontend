...
export default function AssignUserToRoleModal({ isOpen, onClose, onSubmit, role, users }) {
    const [selectedUserId, setSelectedUserId] = useState('');
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState('');
    const assignedUserIds = users.filter(u => u.functionalRole === role?.name).map(u => u.id);
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            await onSubmit(parseInt(selectedUserId), role.name);
            setSelectedUserId('');
            setError('');
            onClose();
        } catch (err) {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-xl w-full max-w-md p-6">
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-2xl font-bold text-slate-900">Assign User to {role.name}</h2>
                ...
                {availableUsers.length === 0 ? (
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                        <p className="text-blue-800 text-sm">All users already have this role assigned</p>
                    </div>
                ) : (
